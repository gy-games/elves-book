...
**站在运维自动化系统实现角度**，运维自动化系统正走在向平台化，更优质的用户体验提升的道路上，往往此类产品均为WEB端或桌面的形式提供运维使用， 这类运维自动化系统若需要与业务操作系统OS或与操作系统上的服务进行交互的时候往往需要自己定义通讯以及调用的实现方式。通过ELVES后，此类运维自动化系统将可以全部面向统一的EVELS API接口，ELVES API的背后为运维所开发的各种业务的实现脚本。![](/assets/system-implemention.png)

**站点运维团队与开发\(前端\)团队合作角度，**运维团队更懂系统，更懂业务并但产品感不强，前端等技能欠缺，开发\(前端\)团队产品感强，有较好的产品实现技能如前端JS,CCS等,但其不熟悉运维业务，更不了解具体业务实现。通过ELVES，开发\(前端\)团队将面向面向的ELVES API接口，运维团队将更加专注的面向脚本，面向具体功能的![](/assets/TIM图片20170704215916.png)
